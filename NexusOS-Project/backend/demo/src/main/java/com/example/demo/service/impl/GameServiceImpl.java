...
import com.example.demo.domain.EmployeeGame;
import com.example.demo.domain.GameBankDaily;
import com.example.demo.domain.GameInvestment;
import com.example.demo.dto.*;
import com.example.demo.mapper.EmployeeGameMapper;
import com.example.demo.mapper.GameBankDailyMapper;
import com.example.demo.mapper.GameInvestmentMapper;
import com.example.demo.mapper.GameRankMapper;
...
public class GameServiceImpl implements GameService {
    ...
    @Autowired
    private com.example.demo.mapper.GameSignMapper gameSignMapper;

    private static final long SIGN_REWARD = 200;

    @Override
    public GameStatusResponse getGameStatus(Long employeeId) {
    ...
    @Override
    @Transactional
    public CollectResponse collectCoins(Long employeeId) {
        EmployeeGame game = getOrCreateGame(employeeId);
        int unclaimedCount = calculateUnclaimedCount(game);
    ...
    @Override
    @Transactional
    public UpgradeResponse upgrade(Long employeeId) {
        EmployeeGame game = getOrCreateGame(employeeId);
        ...
        game.setCoinLevel(game.getCoinLevel() + 1);
        employeeGameMapper.updateByEmployeeId(game);
    ...
    @Override
    public BankTodayResponse getBankToday(Long employeeId) {
    ...
    private BankTodayResponse.LastInvestmentResult getLastInvestmentResult(Long employeeId) {
    ...
    @Override
    @Transactional
    public InvestResponse invest(Long employeeId, InvestRequest request) {
        EmployeeGame game = getOrCreateGame(employeeId);
        LocalDate today = LocalDate.now();
        LocalDateTime now = LocalDateTime.now();
    ...
    @Override
    public RankResponse getRank(Long employeeId) {
    ...
    @Override
    public SignStatusResponse getSignStatus(Long employeeId) {
    ...
    @Override
    @Transactional
    public SignResponse doSign(Long employeeId) {
        getOrCreateGame(employeeId);
        EmployeeGame game = employeeGameMapper.findByEmployeeId(employeeId);
        LocalDate today = LocalDate.now();
    ...
    private EmployeeGame getOrCreateGame(Long employeeId) {
        EmployeeGame game = employeeGameMapper.findByEmployeeId(employeeId);
        if (game == null) {
