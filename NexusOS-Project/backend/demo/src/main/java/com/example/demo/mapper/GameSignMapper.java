...
public interface GameSignMapper {
    ...
    Map<String, Object> findByEmployeeIdAndDate(@Param("employeeId") Long employeeId, @Param("signDate") LocalDate signDate);

    @Select("SELECT COUNT(*) FROM game_sign_log WHERE employee_id = #{employeeId}")
    int getSignCount(@Param("employeeId") Long employeeId);
}
