...
public interface EmployeeGameMapper {
    ...
    EmployeeGame findByEmployeeId(@Param("employeeId") Long employeeId);
    ...
    int addCoins(@Param("employeeId") Long employeeId, @Param("coins") Long coins);

    @Update("UPDATE employee_game SET coins = coins - #{coins} WHERE employee_id = #{employeeId} AND coins >= #{coins}")
    int subtractCoins(@Param("employeeId") Long employeeId, @Param("coins") Long coins);

    @Update("UPDATE employee_game SET last_sign_date = #{lastSignDate}, sign_count = #{signCount} " +
            "WHERE employee_id = #{employeeId}")
    int updateSignInfo(@Param("employeeId") Long employeeId, @Param("lastSignDate") java.time.LocalDate lastSignDate, @Param("signCount") Integer signCount);
